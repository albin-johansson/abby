...
TEST_SUITE("aabb")
{
  ...
  {
    const faabb aabb;
    CHECK(aabb.min == fvec2{0, 0});
    CHECK(aabb.max == fvec2{0, 0});
  }
  ...
  TEST_CASE("aabb::operator==")
  {
    ...
    {
      const faabb fst{{73, 12}, {341, 275}};
      const faabb snd{{27, 63}, {299, 512}};
      CHECK_FALSE(fst == snd);
      CHECK_FALSE(snd == fst);
    }
  }
  ...
  TEST_CASE("aabb::operator!=")
  {
    ...
    SUBCASE("Two different AABBs")
    {
      const faabb fst{{55, 76}, {476, 775}};
      const faabb snd{{29, 44}, {345, 173}};
      CHECK(fst != snd);
      CHECK(snd != fst);
    }
  }
  ...
  TEST_CASE("make_aabb")
  {
    const fvec2 pos{27, 93};
    const fvec2 size{871, 712};
