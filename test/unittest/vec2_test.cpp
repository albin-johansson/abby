...
TEST_SUITE("vec2")
{
  ...
  {
    const vec2 fst{776, 141};
    const vec2 snd{514, 482};
  ...
  TEST_CASE("vec2::operator-")
  {
    const vec2 fst{912, 56};
    const vec2 snd{448, -167};
  ...
  TEST_CASE("vec2::operator!=")
  {
    ...
    SUBCASE("Two different vectors")
    {
      const vec2 fst{82, 76};
      const vec2 snd{148, 897};
      CHECK(fst != snd);
      CHECK(snd != fst);
    }
  }
